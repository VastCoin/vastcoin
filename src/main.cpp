...
extern const unsigned int nTargetSpacing = 1 * 30; // 30 seconds
unsigned int nStakeMinAge = 15 * 60 * 60; // 15 hours
...
const string strMessageMagic = "VastCoin Signed Message:\n";
...
{
		const int HALF_EVERY_BLOCKS = 500;
    int64_t nSubsidy = 4000 * COIN;
    const int halvings = nHeight / HALF_EVERY_BLOCKS;
    ...
    // Subsidy is cut in half every HALF_EVERY_BLOCKS blocks.
    nSubsidy >>= halvings;
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "VastCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "Pressure Grows on Putin as Forensics Experts Reach Crash Victims in Ukraine";
        CTransaction txNew;
        txNew.nTime = 1405959292;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1405959292;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 1327056;
		if(fTestNet)
        {
            block.nNonce   = 1327056;
        }
